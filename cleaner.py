...
class Preprocessing():
  ...
  def __init__(self,data,embeddings,Basic_Clean=True,Contractions=True):
    ...
    self.hashtag = pd.DataFrame()
    self.mention = pd.DataFrame()
    self.url_count = pd.DataFrame()
    self.word_count = pd.DataFrame()
    self.unique_word_count = pd.DataFrame()
    self.stop_word_count = pd.DataFrame()
    self.mean_word_length = pd.DataFrame()
    self.char_count = pd.DataFrame()
    self.punctuation_count = pd.DataFrame()
    
    #Controls
    self.Basic_Clean = Basic_Clean
    self.Contractions = Contractions
    self.more_meta = Basic_Clean
  ...
  def main(self):
    ...
    dpd["id"] = self.data_all["id"]
    self.hashtag = self.data.apply(lambda x: len([c for c in str(x) if c == '#']))
    self.mention = self.data.apply(lambda x: len([c for c in str(x) if c == '@']))
    self.url_count = self.data.apply(lambda x: len([w for w in str(x).lower().split() if 'http' in w or 'https' in w]))
    self.char_count = self.data.apply(lambda x: len(str(x)))
    self.punctuation_count = self.data.apply(lambda x: len([c for c in str(x) if c in string.punctuation]))
    dpd["hashtag"] = self.hashtag
    dpd["mention"] = self.mention
    dpd["url_count"] = self.url_count
    ...
    dpd["text"] = self.data
    dpd["char_count"] = self.char_count
    dpd["punctuation_count"] = self.punctuation_count
    if bool(self.more_meta) == 1:
        self.word_count = self.data.apply(lambda x: len(str(x).split()))
        self.unique_word_count = self.data.apply(lambda x: len(set(str(x).split())))
        self.stop_word_count = self.data.apply(lambda x: len([w for w in str(x).lower().split() if w in STOPWORDS]))
        self.mean_word_length = self.data.apply(lambda x: np.mean([len(w) for w in str(x).split()]))
        dpd["word_count"] = self.word_count
        dpd["unique_word_count"] = self.unique_word_count
        dpd["stop_word_count"] = self.stop_word_count
        dpd["mean_word_length"] = self.mean_word_length
    dpd["target"] = self.data_all["target"]
        
    dpd.to_csv("./mod_csv_data.csv")
    self.dpd = dpd
